	...
	def Compile(self):
		...
		for fname in self.lista:
			...
			if "/" in fname:
				...
				fname=fname.split("/")[-1]
			retcode = os.system("./qc %s/%s"%(self.epath, fname))
			if retcode:
				print("Error %d occured on quest %s/%s"%(retcode, self.epath, fname))
				os._exit(0)
...
def run(filename=LPATH_DFT, w2dump=EPATH_DFT, cp4a=ACOPY_DFT):
	...
	if not filename:
		return False
	fname = open(filename, encoding="ascii", errors="surrogateescape")
	...
	if not len(keys) == 0:
		...
		# dump
		f = open(oname, "w", encoding="ascii", errors="surrogateescape")
		for s in r:
			 f.write(s)
		f.close()
	elif cp4a:
