quest npc_talk2 begin
    state start begin
 		when 20091.chat.locale.epicsura1_chat begin
			...
			say("Arkadaþým, dünya cevaplanmamýþ sorunlarla dolu. Ve")
			say("ben cevap ve bilgi arýyorum. Destansý Silahlarý ")
			say("ve Ruh Taþlarýný hiç duydun mu?")
			end
		when 20091.chat.locale.epicsura2_chat begin
			say_title("Seon Pyeong:")
			say("Bilinmeyeni araþtýrdýðýmý zaten biliyorsundur. Þu")
			say("anda iyi silahlar yapmanýn ve ruh")
			say("taþlarýnýn gücünü arttýrmanýn yolunu arýyorum.")
			end
		when 20091.chat.locale.epicsura3_chat begin
			say_title("Seon Pyeong:")
			say("Ýlginç deðil mi? Bununla ilgili daha fazla")
			say("þeyler öðrenmek istiyorsun deðil mi? Öyleyse bana")
			say("önümüzdeki +9 silahlardan bir tane getir.")
			say("Muharebe Kýlýcý+9, Sarý Ejderha Yayý+9,")
			say("Kurtuluþ Yelpazesi+9, Ejderha Býçaðý+9, Þeytan")
			say("Çýkarma Kýlýcý+9, Cennet ve Dünya Çaný+9 veya")
			say("Hortlak Diþi Kýlýcý+9. Bunun dýþýnda bir adet")
			say("Eskrim Broþürü ve iki adet Kutsama Parþömeni")
			say("lazým. Daha sonra sana destansý bir kýlýç yapmaya")
			say("çalýþacaðým.")
			say("")
			say("")
			say("Ýþe yarayacaðýný garanti edebilirim.")
		end
		...
		when 20003.chat."Ah, ne þirin bir çocuk..." begin
            say_title("Ah-Yu:")
			say("Bir kez daha çýðlýk atýyor... Ne istediðini")
			say("çözemiyorum. þeker falan mý acaba... Ahh hayýr!")
			say("")
			end
		...
		when 20015.chat."Çok güçlü görünüyorsun..." begin
            say_title("Oduncu Deokbae:")
            say("Yýllardan beridir, gücümüzün sýrrý ailemiz için ")
            say("nesilden nesile geçer. Babam bana demiþti ki,")
            say("eðer çan çiçeðini doðru þekilde karýþtýrýrsan")
            say("sana güç verecek bir iksir yapabilirsin. Bildiðim ")
			say("kadarýyla Baek_go bu iksiri yapabiliyor. Tek")
			say("ihtiyacým ona çan çiçeði ")
			say("götürmek.")
			say("")
        end
		...
		when 20087.chat."Sen kimsin?" begin
			say_title("Wonda-Rim:")
			say("")
			say("Ben Wonda-Rim, Chaegirab'ýn yeðeniyim. Amcam")
			say("çok meþhur! Onu tanýyor musun? Bir çok iksiri")
			say("ve bitkiyi araþtýrýyor. Araþtýrmalarý ")
			say("sýrasýnda 100 Bo suyu")
			say("ile Dutu karýþtýrýnca büyülü enerjilere")
			say("karþý çok güçlü bir savunma elde ettiren")
			say("bir iksir bulmayý baþardý. Bildiðim")
			say("kadarýyla bu iksiri Huahn-So yapabiliyor.")
			say("Kendisine gereken malzemeleri götür.")
			say("")
			end
		end
	end
